...
from .encrypt_utils import generate_otp, hash_token, otp_expiration_dt
...
def request_reset_password(request):
    ...
    if user and (not user.scim_external_id or not oidc_enabled):
        ...
        send_password_reset_otp_email(
            user.email, user.username, otp, ip_address
        )
    return Response(
        {
...
def confirm_reset_password(request):
    ...
    user.save()
    send_password_reset_confirm_email(
        user.email, user.username, get_client_ip(request)
    )
    return Response(
